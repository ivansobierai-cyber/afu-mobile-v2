...
export default function OAuthCallback() {
  ...
  // Verifica se o perfil AFU existe após o login bem-sucedido
  ...
  // Redireciona com base no estado do perfil AFU
  useEffect(() => {
    ...
    const perfil = sessionQuery.data?.perfil;
    const destino = perfil ? "/(tabs)" : "/auth/onboarding";
    console.log(`[OAuth] Redirecionando para ${destino} (perfil: ${perfil ? "existe" : "pendente"})`);
    setTimeout(() => {
      router.replace(destino as any);
    }, 800);
  }, [status, sessionQuery.isLoading, sessionQuery.data, router]);
    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        if (params.sessionToken) {
          ...
          setStatus("success");
          console.log("[OAuth] Web authentication successful, aguardando verificação de perfil...");
          return;
        }
        ...
        if (sessionToken) {
          ...
          setStatus("success");
          console.log("[OAuth] Session token stored, aguardando verificação de perfil...");
          return;
        }
        ...
        if (result.sessionToken) {
          ...
          setStatus("success");
          console.log("[OAuth] Authentication successful, aguardando verificação de perfil...");
        } else {
      ...
      <ThemedView className="flex-1 items-center justify-center gap-4 p-5">
        ...
        {status === "success" && (
          <>
            ...
            <Text className="text-sm leading-5 text-center text-muted mt-2">
              {sessionQuery.isLoading ? "Verificando seu perfil AFU..." : "Redirecionando..."}
            </Text>
          </>
        )}
        {status === "error" && (
